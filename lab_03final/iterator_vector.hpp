...
public:
    ...
    Ref operator*() {return (*vector)[i];}

    Ptr operator->(){return &(*vector)[i];}
    ...
    Iterator operator+(ptrdiff_t t) const{return Iterator(vector, i + t);}
    ...
    ptrdiff_t operator-(const Iterator &right) const{return static_cast<ptrdiff_t>(i) - right.i;}

    Ref operator[](ptrdiff_t n){return *(*this + n);
                               }
    bool operator==(const Iterator &right) const{return i == right.i;}
