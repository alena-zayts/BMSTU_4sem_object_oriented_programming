...
{
    std::shared_ptr<BaseSourceLoader> ldr(std::make_shared<FileLoader>());
    std::shared_ptr<BaseModelBuilder> bld(std::make_shared<ModelBuilder>());
    std::shared_ptr<BaseModelLoader> uploader_(std::make_shared<ModelLoader>(ldr, bld));
...
void Controller::TransformCamera(std::string cam_name, Point<double> &move, Point<double> &rotate)
{
    ...
    camera_manager_.TurnX(camera, rotate.getX());
    camera_manager_.TurnY(camera, rotate.getY());
    camera_manager_.TurnZ(camera, rotate.getZ());
}
